...
// Sample images shown in the "See What You Can Create" gallery on the
// landing page. To add/replace a sample:
//   1. Drop your image into the `public/` folder (e.g. public/modern-preview.png)
//   2. Update the `image` path below to match the filename
// The `id` must stay one of 'classic' | 'modern' | 'minimal' to line up
// with the actual template ids used in the resume builder.
const PREVIEW_TEMPLATES = [
  { id: 'classic', label: 'Classic', image: 'img1.png' },
  { id: 'modern', label: 'Modern', image: 'modern-preview.png' },
  { id: 'minimal', label: 'Minimal', image: 'minimal-preview.png' },
];
...
<section className="py-20">
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-12">
        ...
        <a
          href="https://drive.google.com/file/d/1v5IfZWLpxdQmDLoHXxZlIyVQELw_RO4M/view?usp=sharing"
          target="_blank"
  ...
  <div className="max-w-2xl mx-auto">
    <h3 className="text-center text-lg font-semibold mb-4" style={{ color: 'var(--color-foreground-secondary)' }}>
      See What You Can Create
    </h3>

    {/* Template preview tabs — add/replace the image files below in
        public/ to show a different sample for each template. */}
    <div className="flex items-center justify-center gap-2 mb-5">
      {PREVIEW_TEMPLATES.map((tpl) => (
        <button
          key={tpl.id}
          onClick={() => setActivePreview(tpl.id)}
          className="px-4 py-2 rounded-full text-sm font-medium"
          style={{
            backgroundColor: activePreview === tpl.id ? 'var(--color-primary)' : 'var(--color-surface-secondary)',
            color: activePreview === tpl.id ? '#fff' : 'var(--color-foreground-secondary)',
    ...
    <div className="relative group">
      {PREVIEW_TEMPLATES.map((tpl) => (
        <img
          key={tpl.id}
          src={tpl.image}
          alt={`${tpl.label} resume sample created with Momentum Resume`}
          className="w-full rounded-lg shadow-2xl border border-opacity-20 transition-all duration-300"
          style={{
            borderColor: 'var(--color-border)',
            display: activePreview === tpl.id ? 'block' : 'none',
          }}
        />
      ))}
      ...
      <div className="text-center mt-4">
        ...
        <p className="text-lg mb-4" style={{ color: 'var(--color-foreground-secondary)' }}>
          A Project By{' '}
          <a
            href="https://www.linkedin.com/in/aditya-rajkumar-singh/"
            target="_blank"
            rel="noopener noreferrer"
        ...
        <div className="flex justify-center items-center gap-6"> {/* Increased gap for larger icons */}
          ...
          <a
            href="https://github.com/aadi-789"
            target="_blank"
            rel="noopener noreferrer"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-4xl font-bold mb-6" style={{ color: 'var(--color-foreground)' }}>
                ...
                A "skill synapse" suggests connecting skills, which is perfect for a resume project.
              </h2>
            ...
            <div className="surface rounded-lg p-8 text-center">
              ...
              <blockquote className="text-lg italic mb-4" style={{ color: 'var(--color-foreground)' }}>
                "Momentum Resume helped me land my dream job! The LaTeX formatting made my resume look incredibly professional."
              </blockquote>
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl mb-8" style={{ color: 'var(--color-foreground-secondary)' }}>
            Join thousands of professionals who have successfully landed their dream jobs with Momentum Resume.
          </p>
          <button 
            onClick={handleGetStarted}
      ...
      <footer className="surface border-t py-12" style={{ borderColor: 'var(--color-border)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="border-t mt-8 pt-8 text-center text-sm" style={{ borderColor: 'var(--color-border)', color: 'var(--color-foreground-secondary)' }}>
            © 2025 Momentum Resume. All rights reserved.
          </div>
        </div>
      </footer>
    </div>
